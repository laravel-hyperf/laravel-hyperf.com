...
Laravel Hyperf includes Eloquent, an object-relational mapper (ORM) that makes it enjoyable to interact with your database. When using Eloquent, each database table has a corresponding "Model" that is used to interact with that table. In addition to retrieving records from the database table, Eloquent models allow you to insert, update, and delete records from the table as well.
...
To get started, let's create an Eloquent model. Models typically live in the `app\Models` directory and extend the `LaravelHyperf\Database\Eloquent\Model` class. You may use the `make:model` [Artisan command](/docs/artisan) to generate a new model:
...
::: note
Since Eloquent models are query builders, you should review all of the methods provided by Laravel Hyperf's [query builder](/docs/queries). You may use any of these methods when writing your Eloquent queries.
:::
...
As we have seen, Eloquent methods like `all` and `get` retrieve multiple records from the database. However, these methods don't return a plain PHP array. Instead, an instance of `LaravelHyperf\Database\Eloquent\Collection` is returned.

The Eloquent `Collection` class extends Laravel Hyperf's base `LaravelHyperf\Support\Collection` class, which provides a [variety of helpful methods](/docs/collections#available-methods) for interacting with data collections. For example, the `reject` method may be used to remove models from a collection based on the results of an invoked closure:
...
In addition to the methods provided by Laravel Hyperf's base collection class, the Eloquent collection class provides [a few extra methods](/docs/eloquent-collections#available-methods) that are specifically intended for interacting with collections of Eloquent models.

Since all of Laravel Hyperf's collections implement PHP's iterable interfaces, you may loop over collections as if they were an array:
...
```php
use App\Models\Flight;
use LaravelHyperf\Database\Eloquent\Collection;
...
The `cursor` returns an `Hyperf\Collection\LazyCollection` instance. [Lazy collections](/docs/collections#lazy-collections) allow you to use many of the collection methods available on typical Laravel Hyperf collections while only loading a single model into memory at a time:
...
Sometimes you may wish to throw an exception if a model is not found. This is particularly useful in routes or controllers. The `findOrFail` and `firstOrFail` methods will retrieve the first result of the query; however, if no result is found, an `LaravelHyperf\Database\Eloquent\ModelNotFoundException` will be thrown:
...
When interacting with Eloquent models, you may also use the `count`, `sum`, `max`, and other [aggregate methods](/docs/queries#aggregates) provided by the Laravel Hyperf [query builder](/docs/queries). As you might expect, these methods return a scalar value instead of an Eloquent model instance:
...
When assigning JSON columns, each column's mass assignable key must be specified in your model's `$fillable` array. For security, Laravel Hyperf does not support updating nested JSON attributes when using the `guarded` property:
...
If you wish, you may instruct Laravel Hyperf to throw an exception when attempting to fill an unfillable attribute by invoking the `preventSilentlyDiscardingAttributes` method. Typically, this method should be invoked in the `boot` method of your application's `AppServiceProvider` class:
...
You should also add the `deleted_at` column to your database table. The Laravel Hyperf [schema builder](/docs/migrations) contains a helper method to create this column:
...
However, since this can be cumbersome, Laravel Hyperf provides a "higher order" `orWhere` method that allows you to fluently chain scopes together without the use of closures:
...
::: note
Want to broadcast your Eloquent events directly to your client-side application? Check out Laravel Hyperf's [model event broadcasting](/docs/broadcasting#model-broadcasting).
:::
